...
function saveUserRole() {
  ...
  const errorMsg = document.querySelector(".typeErr");

  if (selectedRole) {
    // 2. Save the value ('admin' or 'student') to LocalStorage
    localStorage.setItem("userRole", selectedRole.value);

    // Hide error message if it was showing
    if (errorMsg) errorMsg.style.display = "none";

    return selectedRole.value; // Return value if needed for other logic
  } else {
...
// Example usage: trigger when Register button is clicked
if (submitBtn) {
  submitBtn.addEventListener("click", (e) => {
    // e.preventDefault(); // Uncomment if using a <form> to stop reload
    saveUserRole();
  });
} // Function to save role
function saveUserRole() {
